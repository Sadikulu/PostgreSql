...
göre özet olarak gruplama için kullanılır
Group by cümleciği her grup başına satır döndürür
Group by genelde AVG(), COUNT(), MAX(), MIN(), SUM() gibi aggregate fonksiyonları 
...
--kisi ismine göre satılan toplam 
--meyve miktarlarını gösteren sorguyu yazınız.
...
round(avg(yas),2) as yas_ortalamasi from personel group by ulke,cinsiyet;

-- Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  
--ve maası 30000 den büyük olanları sorgulayınız.
select ulke, cinsiyet, count(*) as calisan_sayisi, 
round(avg(yas),2) as yas_ortalamasi from personel where maas>30000 group by ulke,cinsiyet;
...
select ulke, cinsiyet, count(*) as calisan_sayisi, 
round(avg(yas),2) as yas_ortalamasi from personel where maas>30000 group by ulke,cinsiyet order by yas_ortalamasi desc;
