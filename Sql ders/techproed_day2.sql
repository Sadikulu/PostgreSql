...
--DELETE -TRUNCATE
--TRUNCATE komutu delete gibi bir tablodaki verilerin tamamını siler
--Ancak seçmeli silmez
SELECT * FROM ogrenciler;
...
--perconel tablosununa cinsiye varchar(20) ve yas int şeklinde yeni sutunlar ekleyin
ALTER TABLE personel ADD cinsiyet VARCHAR(20),add yas int;
--personel tablosundan şirket fieldini siliniz
ALTER TABLE personel DROP COLUMN sirket;
--personel tablosundaki sehir sutunun adını ulke olarak değiştirelim
ALTER TABLE personel RENAME column sehir to ulke;
--personel tablosunun adını isciler olarak değiştirin
...
/*Primary Key-->bir sutunun null içermemesini ve sutundaki
verilerin benzersiz olmasını sağlar*/
/*Foregin key-->başka bir tablodaki primeryi keyi referans
göstermek için kullanılır böylelikle tablolar arasında ilişki
kurmuş oluruz*/
/*Unique-->bir sutundaki değerlerin benzersiz yani tek olmasını sağlar*/
/*Not Null-->bir sutunun null içermemesini sağlar
not null kısıtlaması için constranint ismi tanımlanmaz
bu kısıtlama veri türünden hemen sonra yerleştirilir*/
--CHECK--> bir sutan yerleştirilecek değer aralığını sınırlamak için kullanılır
...
CREATE TABLE adresler(
	...
	cadde varchar(30),
	sehir VARCHAR(20),
	CONSTRAINT id_fk FOREIGN key(adres_id) 
