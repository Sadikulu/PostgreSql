...
select * from kisiler order by adres asc;--ASCANDİNG (asc) yazmasakta sıralı verir

-- kisiler tablosunu adres'e göre tersten sıralayarak sorgulayınız.
select * from kisiler order by adres desc;--DESCENDING (desc) tersten sıralama için kullanılır

--kişiler tablosunda maaşı büyükten küçüğe sıralayınız
