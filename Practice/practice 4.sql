...
(select marka_isim from markalar where id>'101');
...
select marka_isim,calisan_sayisi,
(select round(avg(maas),2) from calisanlar where marka_isim=isyeri) 
...
as max_maas,(select min(maas) from calisanlar where marka_isim=isyeri) 
